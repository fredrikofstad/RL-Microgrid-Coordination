...
def baseline_agent_dqn(env, timesteps):
    name = f"PPO-h{env.households}-t{timesteps}"
    start_time = time.time()
    train_dqn(env, timesteps, name)
    end_time = time.time()
    print(f"Time taken: {end_time-start_time}")
    info_matrix = test_model(env, name, DQN)
    return info_matrix
...
if __name__ == "__main__":
    ...
    #ppo_solar = baseline_agent_ppo(env_solar, 50_000)
    #rand_solar = random_actor(env_solar)
    ppo_solar_wind = baseline_agent_ppo(env_solar_wind, 100000)
    ...
    plot_solar_both(env_solar_wind, rand_solar_wind, ppo_solar_wind)
    #plot_energy(env_solar, rand_full, "Energy Generated by the random policy (100 Households)")
    #plot_energy(env_solar, ppo_full, "Energy Generated by the PPO policy (100 Households)")
